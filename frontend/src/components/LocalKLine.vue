<template lang="pug">
  Col
    Card
      p(slot='title') 个股K线
      div(id='localKLine' ref="localKLine" :style="{width:'600px', height:'900px'}")
</template>
...
export default
  name: 'localKLine'
  computed: mapState({
    stockKLine:(state)-> return state.stockKLine
  })
  mounted:()->
    kLineChart = echarts.init document.getElementById('localKLine')
    option =
      title:
        text: name
        left: 0
      ...
      series: [
          ...
          },
          {
            name: 'MA10',
            type: 'line',
            data: calculateMA(10, @stockKLine),
            smooth: true,
            lineStyle: {
              normal: {opacity: 0.5}
            }
          },
          {
            name: 'MA20',
            type: 'line',
            data: calculateMA(20, @stockKLine),
            smooth: true,
            lineStyle: {
              normal: {opacity: 0.5}
            }
          },
          {
            name: 'MA30',
            type: 'line',
            data: calculateMA(30, @stockKLine),
            smooth: true,
            lineStyle: {
              normal: {opacity: 0.5}
            }
          },
          {
            ...
            xAxisIndex: 1,
            yAxisIndex: 1,
            data: @stockKLine.volumes
          }
        ]
    kLineChart.setOption option
  watch:
    stockKLine:->
      kLineChart = echarts.getInstanceByDom(@$refs.localKLine)
      option =
        title:
          text: name
          left: 0
